...
//      y.process = spawn('cvlc', ['--sout-http-mime=video/mpeg','--http-caching=1200', '--sout', '#transcode{vcodec=h264,vb=576,width=360,height=288,acodec=mp4a,ab=96,channels=2}:std{access=http,mux=ts,dst=:' + y.port + '}', y.url,'vlc://quit']);
      y.process = spawn('cvlc', ['--sout-http-mime=video/mpeg','--http-caching=600', '--sout', '#transcode{vcodec=h264,venc=x264{profile=high,level=3,bframes=0,keyint=8},vb=600,fps=25,scale=0.5,acodec=mp4a,ab=32,channels=2,deinterlace}:std{access=http,mux=ts,dst=:' + y.port + '}', y.url,'vlc://quit']);
